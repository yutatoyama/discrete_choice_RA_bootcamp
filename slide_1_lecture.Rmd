...
- 目標１：離散選択モデルによって、どのような分析ができるかを学ぶ。


- 目標２：パッケージを利用した離散選択モデルの推定の基礎を学ぶ。
...
- １コマ目（１３時半から１５時）：講義パート
  - イントロダクション（５分）
  - 離散選択モデルと最尤法の導入（３０分）
  - 事例：きのこの山 VS たけのこの里（１０分）
  - コーディング：`mlogit`パッケージによる推定（４５分）
...
- 上武・遠山・若森・渡辺 [「実証ビジネスエコノミクス」](https://www.nippyo.co.jp/blogkeisemi/available_support_guide/e-biz-econ/)  の第二章
...
-   消費者 $i$ が製品 $j$ から得られる効用
    ...
    -   $p_{j}$: 価格, $X_{j}$: 製品の属性・特徴のベクトル。
    -   $(\alpha,\beta)$: 選好パラメタ
    -   $\epsilon_{ij}$: 個人かつ製品レベルの**ランダムな**選好ショック。価格や製品属性で捉えられない要素、言わば誤差項として考える。
...
-   以下のように効用を定義 $$u_{ij}=V_{ij}+\epsilon_{ij}$$ 
...
- 二項ロジット・プロビットモデル：選択肢が２個 $(J=2)$ の場合


- 多項ロジットモデル： 選択肢の個数が２以上の場合


- ランダム係数ロジットモデル：効用パラメタの係数が何ら頭の分布に従う場合
...
-   この分布の元で、選択確率は
    $$\mathrm{Pr}\left(d_{i}=j \right)=\frac{\exp(V_{ij})}{\sum_{k=1}^{J} \exp(V_{ik})}$$
    -   **ロジット確率**とも呼ばれる。
...
- 二項ロジットモデル：
    $$\mathrm{Pr}\left(d = 1 \right)=\frac{\exp(V_{i1})}{\exp(V_{i1})+\exp(V_{i2})}$$

- 二項プロビットモデル： $\epsilon_{i,j}$ が i.i.d. の標準正規分布に従うと考える。


- 二項ロジット・プロビット(と最尤法)については、`Lecture_9_MLE_binary_choice.pdf` を参照。
...
- ロジットモデルにおける限界効果は
$$
\frac{\partial Pr(d=j)}{\partial p_{j}}=Pr(d=j)\left(1-Pr(d=j)\right)\times\alpha
$$

- 応用例：自己価格弾力性
$$
\frac{\partial Pr(d=j)}{\partial p_{j}} \frac{p_j}{Pr(d=j)}=\left(1-Pr(d=j)\right)\times\alpha p_j
$$
  - 価格が1%変化したときに、需要が何％変化するか？
  

---
## 利用方法２：パラメタの解釈と支払い意思額
...
- 演習において、RAブートキャンプ参加者に回答してもらったアンケートデータを用いた分析を行う。
...
data %>%
  mutate(gender =　case_when(gender == "1 : 男性" ~ "男性", 
    ...
    region =  case_when(region == "1 : 北海道地方" ~ "関東", 
                              ...
                              region == "9 : 海外" ~ "海外"),
    familyhouse = if_else(familyhouse==1,"実家暮らし","実家暮らしでない")) -> data_atr
...
$$
\begin{aligned}
\alpha(z_{i})	&=	\alpha_{0}+\sum_{d=1}^{D}\alpha_{d}z_{di} \\
\beta_{j}(z_{i})	&=	\beta_{0,j}+\sum_{d=1}^{D}\beta_{d,j}z_{di}, j\in \{Kinoko, Takenoko \} 
\end{aligned}
$$
...
```{r, eval=FALSE}
stargazer::stargazer(multilogit,ml_consumer_atr,
...
- ここで、
  ...
  -   $\beta_{i} \sim N\left(\theta_{\beta}, \sigma_{\beta}^{2}\right) .$
...
- 注：消費者属性も加味可能だが講義では省略。(実証ビジエコ第２章を参照)
...
- 乱数のドロー数を便宜上50としているが、これは少なすぎる。


- ドロー数が多いとより正確。ただし、計算時間(推定時間)が長くなる。


- 推定結果がドロー数によってあまり変化しなくなるくらい大きい値にするのが重要。
...
```{r, echo = FALSE}
stargazer::stargazer(multilogit, rcdclogit, type="text", dep.var.caption = "")
```
